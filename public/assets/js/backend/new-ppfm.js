$(document).ready(function(){
	...
	$('#page-4 .f-tambah-data').on('click', function() {
		...
		|| $('#nama-sekolah-t-iv').val() == '' || $('#nisn-t-iv').val() == ''
			...
			$.ajax({
				url: '/ppfm/registrasinew/individu/tanggungan',
				type: 'POST',
				data: formData,
				cache: false,
				contentType: false,
	...
	$('ol').on('click', '#delete-item-art', function(e) {
		$.ajax({
			type: 'GET',
			url: '/delete/pesertaverifindividu?nik='+$(this).attr('data-nik')+'&kode='+$(this).attr('data-kdp'),
			success: function(data){
				msg = $.parseJSON(data);
				if(msg.success === 1){
	...
	$('ol').on('click', '#delete-item-tanggungan', function(e) {
		$.ajax({
			type: 'GET',
			url: '/delete/newpesertatanggungan?nik='+$(this).attr('data-nik')+'&kode='+$(this).attr('data-kdp'),
			success: function(data){
				msg = $.parseJSON(data);
				if(msg.success === 1){
					$(e.currentTarget.offsetParent).remove();
	...
	$('#form-pendaftaran-ppfm').submit(function(e){
		e.preventDefault();
		var asset_var = {},
			rt_var = {},
			arr_rt = [],
	...
	$(document).on('click', '#btn-save-tanggungan', function(){
		...
		|| $('#nama-sekolah-t-iv_'+index).val() == '' || $('#nisn-t-iv_'+index).val() == ''
			...
			$.ajax({
				url: '/ppfm/newupdate/individu/tanggungan',
				type: 'POST',
				data: formData,
				cache: false,
				contentType: false,
	...
	$('#delPeserta').on('click', function(){
        $.ajax({
            type: 'GET',
            url: '/ppfm/peserta-baru/delete/'+ $(this).attr('data-id'),
            success: function(data){
                msg = $.parseJSON(data);
                if(msg.success == 1){
	...
	if($('#cetak-peserta-baru').length !== 0) {
		$('#cetak-peserta-baru').printPage();
	}

	$('#search-kec').on('change', function(){
		location.href = '/ppfm/peserta-baru?kec='+$(this).val();
	});
