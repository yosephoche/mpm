                                            ...
                                            <ul class="uk-pagination">
                                                ...
                                                @if($anggaran->lastPage() > 12)
                                                    ...
                                                    @if($pageNow == 1)
                                                        ...
                                                        @endfor
                                                    @else
                                                        @php($pagelast = ($anggaran->lastPage() - 1) - $pageNow)
                                                        @if($pagelast >= 4)
                                                            @for($i = $pageNow;$i <= $pageNow + 4; $i++)
                                                            <li class="{{ ($pageNow == $i) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$i }}">{{ $i }}</a></li>
                                                            @endfor
                                                        @else
                                                            @if($anggaran->lastPage() == $pageNow)
                                                    ...
                                                    @endif
                                                    <li class="uk-active"><a href="javascript:;">...</a></li>
                                                    <li class="{{ ($pageNow == $anggaran->lastPage()) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$anggaran->lastPage() }}">{{ $anggaran->lastPage() }}</a></li>
                                                @else
                                                    @for($i = 1;$i <= $anggaran->lastPage(); $i++)
                                                        <li class="{{ ($pageNow == $i) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$i }}">{{ $i }}</a></li>
                                                    @endfor
                                                @endif
                                                <li class="{{ ((!empty($_GET['page'])) && ($_GET['page'] == $anggaran->lastPage())) ? 'uk-disabled' : '' }}"><a href="{{ URL('/master/opd').'?page='.(!empty($_GET['page']) ? ($_GET['page'] + 1) : '2') }}"><i class="uk-icon-caret-right"></i></a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    @endif
    ...
    <div class="uk-modal-dialog">
        ...
        <div class="uk-modal-footer">
            <ul>
                ...
                <li><button class="button-red" data-id="" id="delAnggaranKegiatan">Ya</button></li>
            </ul>
        </div>
    </div>
</div>
