                                            ...
                                            <ul class="uk-pagination">
                                                ...
                                                @if($indikator->lastPage() > 12)
                                                    ...
                                                    @if($pageNow == 1)
                                                        ...
                                                        @endfor
                                                    @else
                                                        @php($pagelast = ($indikator->lastPage() - 1) - $pageNow)
                                                        @if($pagelast >= 4)
                                                            @for($i = $pageNow;$i <= $pageNow + 4; $i++)
                                                            <li class="{{ ($pageNow == $i) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$i }}">{{ $i }}</a></li>
                                                            @endfor
                                                        @else
                                                            @if($indikator->lastPage() == $pageNow)
                                                    ...
                                                    @endif
                                                    <li class="uk-active"><a href="javascript:;">...</a></li>
                                                    <li class="{{ ($pageNow == $indikator->lastPage()) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$indikator->lastPage() }}">{{ $indikator->lastPage() }}</a></li>
                                                @else
                                                    @for($i = 1;$i <= $indikator->lastPage(); $i++)
                                                        <li class="{{ ($pageNow == $i) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$i }}">{{ $i }}</a></li>
                                                    @endfor
                                                @endif
                                                <li class="{{ ((!empty($_GET['page'])) && ($_GET['page'] == $indikator->lastPage())) ? 'uk-disabled' : '' }}"><a href="{{ URL('/master/opd').'?page='.(!empty($_GET['page']) ? ($_GET['page'] + 1) : '2') }}"><i class="uk-icon-caret-right"></i></a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    @endif
