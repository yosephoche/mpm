                                    ...
                                    <div>
                                        <table class="uk-table uk-table-hover">
                                            ...
                                            <tbody>
                                                ...
                                                @if (!$kegiatan->isEmpty())
                                                    @foreach($kegiatan as $thisKegiatan)
                                                    <tr>
                                                        <td>{{ $i++ }}</td>
                                                        <td>{{ $thisKegiatan->name }}</td>
                                                        <td>
                                                            <div class="uk-button-group">
                                                                ...
                                                                <div data-uk-dropdown="{mode:'click', pos:'bottom-right'}">
                                                                    ...
                                                                    <div class="uk-dropdown uk-dropdown-small">
                                                                        <ul>
                                                                            <li><a href="{{ URL('/master/jenis-kegiatan/update/').'/'.$thisKegiatan->_id }}">Edit</a></li>
                                                                            <li><a href="javascript:;" data-id="{{ $thisKegiatan->_id }}" id="del-jenis-kegiatan">Hapus</a></li>
                                                                        </ul>
                                                                    </div>
                                                                </div>
                                                            </div>
                                            ...
                                            <ul class="uk-pagination">
                                                ...
                                                @if($kegiatan->lastPage() > 12)
                                                    ...
                                                    @if($pageNow == 1)
                                                        ...
                                                        @endfor
                                                    @else
                                                        @php($pagelast = ($kegiatan->lastPage() - 1) - $pageNow)
                                                        @if($pagelast >= 4)
                                                            @for($i = $pageNow;$i <= $pageNow + 4; $i++)
                                                            <li class="{{ ($pageNow == $i) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$i }}">{{ $i }}</a></li>
                                                            @endfor
                                                        @else
                                                            @if($kegiatan->lastPage() == $pageNow)
                                                    ...
                                                    @endif
                                                    <li class="uk-active"><a href="javascript:;">...</a></li>
                                                    <li class="{{ ($pageNow == $kegiatan->lastPage()) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$kegiatan->lastPage() }}">{{ $kegiatan->lastPage() }}</a></li>
                                                @else
                                                    @for($i = 1;$i <= $kegiatan->lastPage(); $i++)
                                                        <li class="{{ ($pageNow == $i) ? 'uk-active' : '' }}"><a href="{{ URL('/master/opd').'?page='.$i }}">{{ $i }}</a></li>
                                                    @endfor
                                                @endif
                                                <li class="{{ ((!empty($_GET['page'])) && ($_GET['page'] == $kegiatan->lastPage())) ? 'uk-disabled' : '' }}"><a href="{{ URL('/master/opd').'?page='.(!empty($_GET['page']) ? ($_GET['page'] + 1) : '2') }}"><i class="uk-icon-caret-right"></i></a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    @endif
